...
class DoManager():
    def __init__(self, api_token=None, api_version=2):
        self.api_token = api_token
        self.api_endpoint = 'https://api.digitalocean.com/v{}'.format(api_version)

    def show_droplet(self, droplet_id):
        response = requests.get('{}/droplet/{}'.format(self.api_endpoint, droplet_id),
                                headers={'Authorization': 'Bearer {}'.format(self.api_token)})
        return response.json()

    def all_ssh_keys(self):
        response = requests.get('{}/account/keys?per_page=100'.format(self.api_endpoint),
                                headers={'Authorization': 'Bearer {}'.format(self.api_token),
                                         'Content-Type': 'application/json'})
        try:
            ssh_keys = response.json()['ssh_keys']
        except KeyError:
            raise Exception('No ssh keys returned from API')

        keys_fetched = 100
        try:
            total_keys = response.json()['meta']['total']
        except KeyError:
            raise Exception('No count for total ssh keys returned')

        page = 2
        while keys_fetched <= total_keys:
            response = requests.get('{}/account/keys?per_page=100&page={}'
                                    .format(self.api_endpoint, page),
                                    headers={'Authorization': 'Bearer {}'.format(self.api_token),
                                             'Content-Type': 'application/json'})
            try:
                ssh_keys += response.json()['ssh_keys']
    ...
    def all_active_droplets(self):
        response = requests.get('{}/droplets?status=active&per_page=100'.format(self.api_endpoint),
                                headers={'Authorization': 'Bearer {}'.format(self.api_token),
                                         'Content-Type': 'application/json'})
        try:
            droplets = response.json()['droplets']
        except KeyError:
            raise Exception('No droplets returned from API')

        droplets_fetched = 100
        try:
            total_droplets = response.json()['meta']['total']
        except KeyError:
            raise Exception('No count for total droplets returned')

        page = 2
        while droplets_fetched <= total_droplets:
            response = requests.get('{}/droplets?status=active&per_page=100&page={}'
                                    .format(self.api_endpoint, page),
                                    headers={'Authorization': 'Bearer {}'.format(self.api_token),
                                             'Content-Type': 'application/json'})
            try:
    ...
    def new_droplet(self, name, size_id, image_id, region_id, ssh_key_ids, **kwargs):
        droplet_body = {'name': name, 'size': size_id, 'image': image_id, 'region': region_id}
        if ssh_key_ids is not None:
            droplet_body['ssh_keys'] = [int(key) for key in ssh_key_ids]
        droplet_body.update(kwargs)
        response = requests.post('{}/droplets'.format(self.api_endpoint), data=json.dumps(droplet_body),
                                 headers={'Authorization': 'Bearer {}'.format(self.api_token),
                                          'Content-Type': 'application/json'})
        try:
            return response.json()['droplet']
        except KeyError:
            raise Exception('Invalid response returned from droplet creation call')
...
class Droplet(JsonfyMixIn):
    ...
    def setup(cls, api_token):
        cls.manager = DoManager(api_token, api_version=2)

    @classmethod
    def add(cls, name, size_id, image_id, region_id, ssh_key_ids=None, private_networking=False, backups_enabled=False, user_data=None,
            ipv6=False):
        json = cls.manager.new_droplet(name, size_id, image_id, region_id,
                                       ssh_key_ids, private_networking=private_networking,
                                       backups=backups_enabled, user_data=user_data, ipv6=ipv6)
        droplet = cls(json)
        return droplet
...
def main():
    ...
    if not HAS_REQUESTS:
        module.fail_json(msg='requests library required for this module')
